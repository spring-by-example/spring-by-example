...
public class DefaultVisitor implements ApplicationContextAware, ValangVisitor {
    ...
    public Function getFunction(String className, String propName) {
        ...
        if (!StringUtils.hasText(className) || propertyName.startsWith("this") || propertyName.indexOf(".") != -1) {
            ...
            if (propertyName.indexOf(INDEXED_DELIM_BEGIN) != -1) {
                indexedKey = propertyName.substring((propertyName.indexOf(INDEXED_DELIM_BEGIN) + 1), propertyName.indexOf(INDEXED_DELIM_END));
            ...
            property = "get" + org.apache.commons.lang.StringUtils.capitalize(propertyName);
            ...
            if (hGeneratedBeanFunctions.containsKey(key)) {
                ...
                try {
                    Method method = ReflectionUtils.findMethod(ClassUtils.forName(className), property);
                    ...
                    if (primitiveClass != null) {
                        generatedMethod.append(");");
                    } else if (methodReturnType.isAssignableFrom(Map.class)) {
                        generatedMethod.append(".get(\"" + indexedKey + "\")");
                    } else if (methodReturnType.isAssignableFrom(List.class)) {
                        generatedMethod.append(".get(" + indexedKey + ")");
